...
public class FlowChartChooser { // extends Command {
	...
	private CommandGroup autoRun;

	public FlowChartChooser(SendableChooser<Command> ourSide, SendableChooser<Command> primaryObjective,
			SendableChooser<Command> crossMiddle, SendableChooser<Command> enableSecondary) {
	...
	private boolean CodeMatch(String codes, String match, int position) {
		...
		return (match.toCharArray()[index + 1] == wildcard
				|| match.toCharArray()[index + 1] == codes.toCharArray()[index + 1])
				&& (match.toCharArray()[index + 2] == wildcard
						|| match.toCharArray()[index + 2] == codes.toCharArray()[index + 2]);
	}
